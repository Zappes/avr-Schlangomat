...
static inline void am2302_sda_out(uint8_t sensor) {
	DDR_SENSOR |= (1 << sensor);
}
...
static inline uint8_t am2302_is_hi(uint8_t sensor) {
	return PIN_SENSOR & (1 << sensor);
}

static inline uint8_t am2302_is_low(uint8_t sensor) {
	return !(PIN_SENSOR & (1 << sensor));
}
